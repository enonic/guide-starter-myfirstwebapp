...
. Update your `webapp.js` and `hello.html` as follows:
+
.src/main/resources/webapp/webapp.js
[source,js]
----
...
----
+
. To make the router library available for our app, add the following line to the dependencies section of the `build.gradle` file.
+
...
NOTE: The pageUrl() view function is used to dynamically generate server relative URLs for the pages. This eliminates the need for dealing with relative links etc.
